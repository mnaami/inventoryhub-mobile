...
class _PurchaseOrderEditScreenState
      ...
      builder: (_) => ListView(
        children: [
          for (final p in products)
            ListTile(
                title: Text(p.name),
                subtitle: Text(p.purchasePrice.toStringAsFixed(2)),
  ...
  Future<void> _save() async {
    ...
    try {
      await ref.read(purchaseOrderServiceProvider).createDraft(
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: ListView(
        ...
        children: [
          ListTile(
            title: Text(_supplier?.name ?? 'Select supplier'),
          ...
          for (final line in _lines)
            ListTile(
              ...
              subtitle:
                  Text('${line.product.purchasePrice.toStringAsFixed(2)} each'),
              trailing: SizedBox(
                width: 80,
                child: TextFormField(
